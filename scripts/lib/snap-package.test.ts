...
test("Snap metadata requires the reviewed strict native contract", () => {
  assert.equal(snapArtifactName("0.2.18", "arm64"), "dsh-desktop-community_0.2.18_arm64.snap");
  assert.deepEqual(snapMetadataProblems(metadata, { version: "0.2.18", architecture: "arm64" }), []);
  assert.ok(
  ...
  assert.ok(
    snapMetadataProblems(
      ...
      { version: "0.2.18", architecture: "arm64" },
    ).some((problem) => problem.includes("WebKit layout")),
  );
  const amd64Metadata = metadata
    .replace("- arm64", "- amd64")
    .replaceAll("aarch64-linux-gnu", "x86_64-linux-gnu");
  assert.deepEqual(snapMetadataProblems(amd64Metadata, { version: "0.2.18", architecture: "amd64" }), []);
});
